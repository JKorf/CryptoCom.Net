...
V2 uses a strict interface per capability. The `SharedApi` property on each API surface therefore exposes only the operations or subscriptions that surface actually supports. `ICryptoComSharedApiClient` groups the exchange's Shared API surfaces for dependency injection and runtime capability lookup.
...
using var restClient = new CryptoComRestClient();
...
* Version 4.5.0 - 24 Sep 2026
    ...
    * Shared APIs
      ...
      * Added `(I)CryptoComSharedApiClient` containing all V2 SharedApi references
      * Added `SharedApi` to `CryptoComOptions` to provide SharedApi related options
    ...
    * Added OnChainWithdrawal value to TransactionType enum
